...
def barplot(x):
    ...
    ax.set_ylabel('Avg. Norm of Est.\n Error per learner', fontsize=15)
...
for i in range(len(topology)):
    # fname = 'Result_smallrate/Result2_more_' + topology[i]
    fname = 'Result_smallrate/beta_more_' + topology[i]
    result = readresult(fname)
    for j in range(len(algorithm)):
        # obj[algorithm[j]][topology_short[i]] = result[algorithm_map[j]][1]/ result[0][1]
        obj[algorithm[j]][topology_short[i]] = result[algorithm_map[j]]/result[0]
