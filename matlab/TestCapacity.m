% We decrease the link capacity of the links to learner 1
...
[Lambda_Alpha{capacity,1}, lambda_e1] = MaxAlpha(TWINDi,TWINDj,TW,V,lambda_s,L,p,Learner,Source,10);

[Lambda_Sum{capacity,1}, lambda_e2] = MaxAlpha(TWINDi,TWINDj,TW,V,lambda_s,L,p,Learner,Source,0);
...
Lambda_FW{capacity,1} = lambda_fw;
...
Lambda_FW_fast{capacity,1} = lambda_fwl;

% Projected Gradient
lambda_pg = Lambda_Sum{capacity,1};
for kk = 1:K
    % Estimate the current gradient
...
Lambda_GP{capacity,1} = lambda_pg;
toc
end
save('Abilene_capacity.mat')