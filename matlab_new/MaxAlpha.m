function [vk, lambda_e] = MaxAlpha(TWINDi,TWINDj,TW,V,lambda_s,L,p,Learner,Source,alpha,t_l)
...
cvx_begin quiet
    ...
    for l = 1:L
        ...
        sum_l = 0;
        for nl = 1:n_l
            sum_l = sum_l + sum(sum(lambda_e(in_l(nl),:,:)));
        end
        gain = gain + pow_p(sum_l, 1-alpha)/(1-alpha); 
    end
    maximize(gain);
    subject to
        for e=1:E
